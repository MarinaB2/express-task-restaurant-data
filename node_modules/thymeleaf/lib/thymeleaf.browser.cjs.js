  ...
  process(element, attribute, attributeValue, context) {
    ...
    if (/(.+=.+,)*.+=.+/.test(attributeValue)) {
      attributeValue.split(',').forEach(attribute => {
        ...
        element.setAttribute(attributeParts[0], stringUtils.escapeHtml(context.expressionProcessor.process(attributeParts[1], context)));
      });
    }
...
async function extractFragment(dialectPrefix, fragmentInfo, context) {
  ...
  if (templateResolver) {
    ...
    return dumbQuerySelector.$(`[${dialectPrefix}\\:${NAME$5}^="${fragmentName}"]`, template) || dumbQuerySelector.$(`[data-${dialectPrefix}-${NAME$5}^="${fragmentName}"]`, template);
  }
